...
## Both the present script and the raw data file need to be saved to the R
## working directory before running. Please do not change the name of the data
## file or else the script will not work.

## The user needs to input information in two places:

## 1. IN THE PRESENT SCRIPT
## Scroll down to the editable section of the present script and key in a unique
## title for the analysis as a character string (don't forget the "" quotes).
## While this step is optional, giving a unique, informative title is
## recommended because the title is used in automatically generating the name of
## the output files.

## 2. IN THE RAW DATA FILE
## The easiest way to insert the data is to do copy-paste from Excel. For this,
## you need to fill in three columns of an Excel worksheet as shown below:
...
## Save the `probit_raw_data_en.txt` before running the present script in
## RStudio (Code>Source file...) or in R (File>Source file...). Alternatively,
## enter the following code on the command line:   source("probit_en.R")

## If the script runs successfully, the message below is printed to screen:
...
## Lastly, you can ignore any warning messages like this:
...
probdata <- read.table(here("code", "analysis_code", "qPCR", "LOD_LOQ", "vv.txt"), header = TRUE)
...
# ----------------------- GRAPH 1, 0.1 to 100000 copies --------------------------

## The code below saves a plot of the probit model to a .pdf file in the R
## working directory.
...
text(EDose[EDose[, 1] == 50, 2], 0.5,
     labels = signif(EDose[EDose[, 1] == 50, 2], digits = 3), cex = 0.875,
     pos = 2, col = coltext, offset = 1.05)
text(EDose[EDose[, 1] == 95, 2], 0.95,
     labels = signif(EDose[EDose[, 1] == 95, 2], digits = 3), cex = 0.875,
     pos = 2, col = coltext, offset = 1.05)

dev.off()

# -------------------- GRAPH 2, 0.1 to 100000 copies, A4 paper -----------------
