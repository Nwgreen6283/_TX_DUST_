...
vc3<- vc1 %>%
  ...
  mutate(point_shape = case_when((LOD == 3 & site == "Canals") ~ 1,
         ...
         per_pos = case_when(LOD == 5 ~ 0,
                             LOD == 4 ~0.2,
                             LOD == 3 ~ 0.4,
                             LOD == 2 ~ 0.6,
                             LOD == 1 ~ 0.8,
                             LOD == 0 ~ 1)) %>% #If all three or 5 were below LOD (so n = 3 or n = 5), then it will be shape #1 or open circle. Else it will be shape #16 or closed cirlce
  unique()
