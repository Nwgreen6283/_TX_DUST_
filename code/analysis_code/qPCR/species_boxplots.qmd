...
ggplot(vv_sum, 
       aes(x=date, 
           y=log10(copies_mL), 
           color=site)) + 
    geom_errorbar(data = vv_sum[vv_sum$point_shape == "16",],
      aes(ymin=log10(copies_mL-se), 
          ymax=log10(copies_mL+se)), 
      width=0.3) +
    geom_errorbar( data = vv_sum[vv_sum$point_shape == "1" ,],
      aes(ymin=log10(0), 
          ymax=log10(0), 
      width=0)) +
    geom_line() +
    geom_point(data = vv_sum, shape= vv_sum$point_shape, size = 2)+
  theme_bw()+
  labs(x = "Date",
       y = "log10(V. vulnificus)",
       color = "Site",
       caption = "Open circles = replicates were non-detects")+
...
ggplot(vc_sum, 
       aes(x=date, 
           y=log10(copies_mL), 
           color=site)) + 
    geom_errorbar(data = vc_sum[vc_sum$point_shape == "16",],
      aes(ymin= log10(copies_mL-se), 
          ymax=log10(copies_mL+se)), 
      width=0.3) +
    geom_errorbar( data = vc_sum[vc_sum$point_shape == "1" ,],
      aes(ymin=0, 
          ymax=0), 
      width=0) +
    geom_line() +
    geom_point(data = vc_sum, shape= vc_sum$point_shape, size = 2)+
  theme_bw()+
  labs(x = "Date",
       y = "log10(V. cholerae)",
       color = "Site")+
