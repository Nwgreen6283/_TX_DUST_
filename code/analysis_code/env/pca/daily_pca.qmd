...
master<-
  master %>%
  select(!c(do_per, do_mgl, pH, secchi, din_dip, toc, cfu_gre, cfu_yel))
```
...
```{r}
daily_pca <- prcomp(master[,c(-23,-1)], #283 is site column
                   scale=TRUE)
```
...
fviz_pca_biplot(daily_pca, 
                ...
                col.var = "deepskyblue",
                habillage  = PC$site,
                title = "Biplot of all Sites", geom="point") +
  theme_bw()+
...
c2_d<-
  ...
  mutate(site = "C2",
         date = as.Date(date),
         date = format(date, format = "%d")) %>%
  select(!c(do_per, do_mgl, pH, secchi, din_dip, toc, cfu_gre, cfu_yel, hf_copies_mL, X, site))
...
```{r}
bo_pca <- prcomp(bo_d[,-1], #Exclude Date column
                   scale=TRUE)
bo_PC<- as.data.frame(bo_pca$x)
bo_PC$date<-bo_d$date

c2_pca <- prcomp(c2_d[,-1], #Exclude Date column
                   scale=TRUE)
c2_PC<- as.data.frame(c2_pca$x)
c2_PC$date<-c2_d$date

rd_pca <- prcomp(rd_d[,-1], #Exclude Date column
                   scale=TRUE)
