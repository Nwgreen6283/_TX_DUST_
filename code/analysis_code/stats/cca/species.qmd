...
```{r}
sp<- readRDS(file = here("data", "processed_data", "species_copies.rds"))
...
vc_bo<- 
  master %>%
  filter(site == 'Blind Oso' & type =="vc")

vc_c2<- 
  master %>%
  filter(site %in% "Canals" & type %in% "vc")
vc_rd<- 
  master %>%
  filter(site %in% "Gulf" & type %in% "vc")

#Vv
vv_bo<- 
  master %>%
  filter(site %in% "Blind Oso" & type %in% "vv")
vv_c2<- 
  master %>%
  filter(site %in% "Canals" & type %in% "vv")
vv_rd<- 
  master %>%
  filter(site %in% "Gulf" & type %in% "vv")

#Vp
vp_bo<- 
  master %>%
  filter(site %in% "Blind Oso" & type %in% "vp")
vp_c2<- 
  master %>%
  filter(site %in% "Canals" & type %in% "vp")
vp_rd<- 
  master %>%
  filter(site %in% "Gulf" & type %in% "vp")
```
