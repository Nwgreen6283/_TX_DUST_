...
### Clean and Trim. Log-Transform Dust 
...
```{r}
cor.test(bo2$lag, bo2$copies_mL)

cor.test(c22$lag, c22$copies_mL)

cor.test(rd2$lag, rd2$copies_mL)
```
...
fig1<-
  ...
  stat_correlation(method = "pearson",label.x = 0.05,label.y = 0.75, small.r = TRUE,size = 3.5)+
  annotate("text", x = 0.0335, y = 55000, label = "p = 0.01", size =3.5)+
...
#CANALS
 fig2<-
  ...
  stat_correlation(method = "pearson", label.x = 0.05,label.y = 0.75,small.r = TRUE, size = 3.5)+
  annotate("text", x = 0.035, y = 23000, label = "p = 0.003", size = 3.5)+
...
fig3<-
  ...
  stat_poly_eq(aes(x = lag, y = copies_mL),rr.digits = 4, size = 3.5)+
  stat_correlation(method = "pearson",label.x = 0.05,label.y = 0.75, small.r = TRUE, size = 3.5)+
  annotate("text", x = 0.0335, y = 85000, label = "p = 0.02", size = 3.5)+
...
```{r}
bo2<- bo %>% mutate(
  cfu_lag =Lag(bo$t7sum, shift  = 0)) %>% #Make lag from ccf
filter(between(date, as.Date('2022-07-07'), as.Date('2022-07-19')))

c22<- c2%>% mutate(
  cfu_lag =Lag(c2$t7sum, shift  = 0)) %>% #Make lag from ccf
filter(between(date, as.Date('2022-07-07'), as.Date('2022-07-19')))

rd2<- rd%>% mutate(
  cfu_lag =Lag(rd$t7sum, shift  = 1)) %>% #Make lag from ccf
filter(between(date, as.Date('2022-07-07'), as.Date('2022-07-19')))
```

#### Get Pearson's correlation p-value to put on plot

**Also: CFU distribution is normal, so that is why I am using Pearson's here.**

```{r}
cor.test(bo2$cfu_lag, bo2$cfu_tot)

cor.test(c22$cfu_lag, c22$cfu_tot)

cor.test(rd2$cfu_lag, rd2$cfu_tot)
```

```{r}
#BLIND OSO
fig1<-
  bo2 %>% ggplot(aes(x = cfu_lag, y = cfu_tot)) +
  geom_point() +
  stat_smooth(method = "lm")+
  stat_poly_eq(aes(x = cfu_lag, y = cfu_tot),
               rr.digits = 4, size = 3.5)+
  stat_correlation(method = "pearson",label.x = 0.05,label.y = 0.75, small.r = TRUE,size = 3.5)+
  annotate("text", x = 0.032, y = 500, label = "p = 0.02", size =3.5)+
  labs(title = "Blind Oso (0-day lag to dust)",
       x = "log10(dust)",
       y = "CFU/mL")+
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
#CANALS
 fig2<-
   c22 %>% ggplot(aes(x = cfu_lag, y = cfu_tot)) +
  geom_point() +
  stat_smooth(method = "lm")+
  stat_poly_eq(aes(x = cfu_lag, y = cfu_tot), rr.digits = 4, size = 3.5)+
  stat_correlation(method = "pearson", label.x = 0.05,label.y = 0.75,small.r = TRUE, size = 3.5)+
  annotate("text", x = 0.032, y = 600, label = "p = 0.47", size = 3.5)+
  labs(title = "Canals (0-day lag to dust)",
       x = "log10(dust)",
       y = "CFU/mL") +
   theme_bw()+
  theme(axis.text.x = element_blank(),
...
#GULF  
fig3<-
  rd2 %>% ggplot(aes(cfu_lag, cfu_tot)) +
  geom_point() +
  stat_smooth(method = "lm")+
  stat_poly_eq(aes(x = cfu_lag, y = cfu_tot),rr.digits = 4, size = 3.5)+
  stat_correlation(method = "pearson",label.x = 0.05,label.y = 0.75, small.r = TRUE, size = 3.5)+
  annotate("text", x = 0.032, y = 1000, label = "p = 0.02", size = 3.5)+
  labs(title = "Gulf (1-day lag to dust)",
       x = "log10(dust)",
       y = "CFU/mL")+
   theme_bw() +
