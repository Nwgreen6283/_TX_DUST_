...
```{r}
data <-read_csv(here("data", "raw_data", "qPCR", "total", "total_copies_mon_daily.csv"))
```
...
Add Dilution Column and Filter column. The dilution column applies if samples were diluted prior to qPCR. For example, if I diluted a sample 1:10, my dilution column would denote "10". The filter column pertains to the amount of water sample (mL) that was concentrated onto a water filter prior to qPCR.
...
copies_daily<- filter_daily %>%
    ...
    sq = 10^log_sq, #This is in copies per ul
    copies_uL_extract = sq *dilution, #(Copies/uL of 1:10)*(1 if there was no sample dilution // *10 if there was a 1:10 dilution etc.)
    copies_assay = copies_uL_extract * 2.5,
    copies_assay= case_when(copies_assay == 0 ~ 1.86, #50% LOD
                     TRUE ~ copies_assay),
    copies_uL_extract = copies_assay / 2.5,
...
Some samples did not amplify and have no values: Replace NaN with 0 for copies/mL
...
Date Column: This adds a date corresponding the a number in the sample name.
...
#### Also keep triplicates for ANOVA

```{r}
write_csv(date_daily, here("data", "processed_data", "qpcr", "ANOVA Data", "daily_anova_data.csv"))
```
...
write.csv(copies_final,file= here("data","processed_data", "qpcr","total_clean_copies.csv")) 
