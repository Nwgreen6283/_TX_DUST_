...
d<- d %>%
    ...
    copies_assay= case_when((copies_assay == "NaN" | copies_assay < 8.4960) ~ 4.25, #50% LOD
                     TRUE ~ copies_assay),
...
per_pos<- d %>% 
 ...
 #group_by(Sample) %>% #Group all chr together. All that should be left out is the numeric vector we want to work with (in this case, copies_mL)
  ...
  #ungroup() %>%
  mutate(lod = case_when(copies_assay < 8.496 ~ "bad", TRUE ~"good"))

write_csv(per_pos,here('data', 'processed_data', 'qpcr', 'LOD', 'va_copies_assay.csv'))
```
