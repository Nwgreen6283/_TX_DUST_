...
copies_redo<- filter_redo %>%
    ...
    copies_assay = copies_uL_extract * 1.2,
    copies_assay= case_when((copies_assay == "NaN"| copies_assay < 0.1092)~ 0.546, #50% LOD
                     TRUE ~ copies_assay),
    copies_uL_extract = copies_assay / 1.2,
...
per_pos<- copies_redo %>% 
 ...
 # group_by(Sample) %>% #Group all chr together. All that should be left out is the numeric vector we want to work with (in this case, copies_mL)
  #summarize_if(is.numeric, mean) %>%
  #ungroup() %>%
  mutate(lod = case_when(copies_assay < 1.092 ~ "bad", TRUE ~"good")) %>%
  drop_na()

write_csv(per_pos,here('data', 'processed_data', 'qpcr', 'LOD', 'vv_copies_assay.csv'))
```
