...
copies<- filter %>%
   mutate(
    ...
    copies_assay= case_when((copies_assay == "NaN" | copies_assay < 0.8064) ~ 0.4032, #50% LOD
                     TRUE ~ copies_assay),
...
#summarize_if(is.numeric, mean) %>%
 # ungroup() %>%
  mutate(lod = case_when(copies_assay < 0.8064 ~ "bad", TRUE ~"good")) %>%
  drop_na()

write_csv(per_pos,here('data', 'processed_data', 'qpcr', 'LOD', 'vp_copies_assay.csv'))
```
